...
func ParseQueryParameter(request *http.Request) *Query {
	query := New()

	limit, err := strconv.Atoi(request.URL.Query().Get(ParameterLimit))
	// equivalent to undefined, use the default value
	if err != nil {
		limit = -1
	}
	page, err := strconv.Atoi(request.URL.Query().Get(ParameterPage))
	// equivalent to undefined, use the default value
	if err != nil {
		page = 1
	}
	...
	query.SortBy = Field(defaultString(request.URL.Query().Get(ParameterOrderBy), FieldCreationTimeStamp))

	ascending, err := strconv.ParseBool(defaultString(request.URL.Query().Get(ParameterAscending), "false"))
	if err != nil {
	...
	query.LabelSelector = request.URL.Query().Get(ParameterLabelSelector)
